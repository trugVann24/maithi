<x-app-layout>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
            ...
            <div class="sm:flex sm:items-center sm:justify-between">
                ...
                <h2 class="text-lg font-medium text-gray-800 ">Danh sách quyền truy cập</h2>
                ...
                <div class="flex items-center justify-center mt-4 gap-x-3">
                    ...
                    <a href="{{route('admin.permissions.create')}}" class="flex items-center justify-center w-1/2 px-5
                    py-2
                    text-sm
                    tracking-wide
                    text-white
                                    ...
                                    <tr>
                                        <td class="px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                                            <div class="inline-flex items-center gap-x-3">
                                                <input type="checkbox" class="text-blue-500 border-gray-300 rounded "/>
                                                <div>
                                                    <p class="font-normal text-gray-800">{{__($permission->id)}}</p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-4 py-4 text-sm text-gray-500 whitespace-nowrap">
                                            ...
                                            justify-center hover:text-blue-600">
                                                ...
                                                <form action="{{ route('admin.permissions.destroy',
                                                    $permission->id) }}"
                                                      method="POST"
                                                      class="deleteForm" data-permission-id="{{ $permission->id }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" data-open-modal="deletePermission"
                                                            data-permission-id="{{ $permission->id }}" class="w-9 h-9 rounded-full hover:bg-red-50 flex items-center
    ...
    <x-modal name="deletePermission" maxWidth="lg">
        <div class="p-4">
            <h2 class="py-2 text-base font-semibold uppercase">Xác nhận xoá !</h2>
            <p class="text-sm font-medium">Bạn có chắc chắn muốn xoá quyền này không?</p>
        ...
        <script>
            $(document).ready(function () {
                $('[data-open-modal]').click(function () {
                    ...
                    $('#modal-' + modalName).show().attr('data-permission-id', permissionId);
                });
                ...
                $('#confirmDelete').click(function () {
                    var modal = $('#modal-deletePermission');
                    var permissionId = modal.data('permission-id');
                    $('.deleteForm[data-permission-id="' + permissionId + '"]').submit();
                });
            });
        </script>
    @endpush
